...
function getBrandCatalogHref(brandName: string): string {
  return `/categorias?q=${encodeURIComponent(brandName)}`;
}
    ...
    <section className="py-12">
      <div className="mb-8 text-center">
        <h2 className="font-heading text-2xl font-bold text-text">
          Nuestras <span className="text-accent">Marcas</span>
        </h2>
      ...
      <div className="flex flex-wrap items-center justify-center gap-8 md:gap-12">
        {brands.map((brand) => (
          <div key={brand.id} className="group flex flex-col items-center gap-2">
            <a
              href={getBrandCatalogHref(brand.name)}
              onClick={() =>
