...
import { deleteCategory } from "@/app/(admin)/admin/categorias/actions";
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40" onClick={onClose}>
      ...
      >
        ...
        <p className="mt-2 text-sm text-muted">
          ...
          <strong className="text-text">&ldquo;{categoryName}&rdquo;</strong>?
        ...
        {(hasProducts || error) && (
          <p className="mt-3 rounded-lg bg-error/10 px-3 py-2 text-sm text-error">
            {error ??
              `No se puede eliminar: ${productCount} producto${productCount === 1 ? "" : "s"} asociado${productCount === 1 ? "" : "s"}. Mueva o elimine los productos primero.`}
          </p>
        )}
