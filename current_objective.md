...
- **Story:** Como administradora, quiero ver KPIs clave, actividad reciente y accesos rápidos en el dashboard, para poder entender el estado del negocio y ejecutar tareas frecuentes sin navegar múltiples pantallas.
- **Spec Level:** Standard
- **Branch:** `hu/6.1`
...
- **Type:** [CC]
- **Cycle:** IMPLEMENT → TEST → REFACTOR
- **Files:** `src/components/admin/AdminKpiCards.tsx` (new), `src/components/admin/AdminRecentActivity.tsx` (new), `src/components/admin/AdminQuickActions.tsx` (new)
- **Verification:** `npm run test:run -- src/tests/integration/hu-6.1-scenarios.test.tsx`

### Task 4: Integrar accesos rápidos del dashboard con módulos prioritarios (~40 min)
- **Type:** [SC]
- **Cycle:** IMPLEMENT → TEST → REFACTOR
- **Files:** `src/app/(admin)/admin/page.tsx`, `src/components/admin/AdminQuickActions.tsx`
- **Verification:** `npm run test:run -- src/tests/integration/hu-6.1-scenarios.test.tsx`
...
- [ ] Ingresar a `/admin` autenticada y validar que se muestran KPI cards para productos, categorías, recetas y marcas/banners.
- [ ] Verificar que los conteos KPI sean consistentes contra datos reales esperados en panel (comparación rápida con módulos).
- [ ] Confirmar que existe sección de actividad reciente con elementos (si hay datos) y con estado vacío amigable (si no hay).
- [ ] Probar “Acciones rápidas” y confirmar navegación a `/admin/productos`, `/admin/categorias`, `/admin/recetas` y `/admin/marcas`/`/admin/banners`.
- [ ] Simular ausencia parcial de datos (o revisar estado vacío) y validar degradación controlada sin romper dashboard.
- [ ] Revisar visual responsive básico en desktop y mobile sin overflow/solapamientos.
...
- [ ] All changes committed with `feat(HU-6.1):` convention
- [ ] Tag `HU-6.1` created
